...
import { FaBookMedical, FaPowerOff, FaRegUserCircle } from 'react-icons/fa';
...
const Sidebar = () => {
  ...
  return (
    <>
      ...
      {/* Sidebar drawer */}
      <nav
        className={`fixed top-0 left-0 z-20 h-screen font-light flex flex-col justify-between border-r-[1px] bg-white transition-transform duration-300 w-64
      ...
      >
        <div>
          <div className="flex items-center h-12 px-5 py-3 overflow-hidden">
            ...
            <span className="text-xs self-end mb-1 ml-2 text-gray-600 truncate">Doctor View</span>
          </div>
          <div className="border-b-[1px]"></div>
          <div className="px-5 py-3">
            <ul className="py-2 font-medium space-y-3 text-medium">
              <Link to="/dashboard" className={`flex items-center gap-2 py-1 rounded ${isActive('/dashboard')}`}>
                <MdDashboard className="" /> Dashboard
              </Link>
              <Link to="/patients" className={`flex items-center gap-2 py-1 rounded ${isActive('/patients')}`}>
                <LuUsers /> Patients
              </Link>
              <Link to="/labs" className={`flex items-center gap-2 py-1 rounded ${isActive('/labs')}`}>
                <MdBloodtype /> Labs
              </Link>
              <Link to="/reports" className={`flex items-center gap-2 py-1 rounded ${isActive('/reports')}`}>
                <TbReportAnalytics /> Reports
              </Link>
              <Link to="/analytics" className={`flex items-center gap-2 py-1 rounded ${isActive('/analytics')}`}>
                <PiChartBarFill /> Analytics
              </Link>
              <Link to="/settings" className={`flex items-center gap-2 py-1 rounded ${isActive('/settings')}`}>
                <IoMdSettings /> Settings
              </Link>
              <Link to="/ai-knowledge" className={`flex items-center gap-2 py-1 rounded ${isActive('/ai-knowledge')}`}>
        ...
        <div className="px-2 pb-2 flex items-center justify-between gap-2 text-medium font-normal text-black border-t-[1px] pt-2">
          <div className="flex items-center gap-2">
            ...
            <Link to="/settings" className="cursor-pointer hover:text-gray-200 text-sm font-medium text-medium">
